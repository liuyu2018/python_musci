...
def download_mp3(url,filename,save_path):
    ret_flag = False
    if not len(save_path):
        abspath = os.path.abspath('.')  # 获取绝对路径
        os.chdir(abspath)
        # response = requests.get(url).content
        save_path = os.path.join(abspath,filename+'.mp3')
    else:
        save_path = os.path.join(save_path,filename+'.mp3')
    try:
        urllib.request.urlretrieve(url, save_path)
        print('下载完毕,可以在%s  路径下查看' % save_path) 
        ret_flag = True
    except:
        print('下载%s.mp3失败' % save_path) 
    
    return [ret_flag,save_path]
